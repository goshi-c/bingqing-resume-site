...
表达边界：
- 帆软经历只能说“参与模型能力评测与 NLP 任务复现”“围绕 Qwen2.5-7B 设计测试维度”“沉淀评测记录和 Badcase 口径”，不要说“负责整个模型评测体系”“主导算法项目”。
- 数字人 AI 名片只能说“产品方案设计、作品管理模块梳理、原型和需求文档输出、支撑体验/交付讲解”，不要说“完整负责商业化上线”。
- Paper Pilot 可以说“独立设计并搭建 Web MVP”，但不要说“商业化上线”“真实大规模用户验证”。
- 没有证据的范围、收益、上线、团队管理、研发主导表述都要避免。
`;
...
export async function onRequestPost(context) {
  ...
  const endpoint = env.DEEPSEEK_API_URL || "https://api.deepseek.com/chat/completions";
  const model = env.DEEPSEEK_MODEL || "deepseek-v4-flash";

  if (!apiKey) {
    return json({ error: "DeepSeek API Key 未配置。" }, { status: 500 });
  }
  ...
  try {
    ...
    if (!answer) {
      return json({ error: "DeepSeek 返回格式无法解析。" }, { status: 502 });
    }

    return json({ answer });
  } catch {
    return json({ error: "数字分身服务请求失败。" }, { status: 500 });
  }
}
